...
def train_lstm_model(
    ...
    df = df.fillna(0)
    df = df.drop(columns=[col for col in ['timestamp', 'wearing_off_id', 'wearing_off_start', 'wearing_off_end'] if col in df.columns])

    x=df.drop(columns=[col for col in ['wearing_off', 'time_from_last_drug_taken'] if col in df.columns])
